  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error('Subtask save error:', result);
      }
    } catch (error) {
      console.error('Error while saving subtask:', error);
    ...
    <Dialog open onClose={onClose} PaperProps={{
      ...
      <DialogContent sx={{
        ...
        <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2} pt={4}>
          <TextField label="Project" fullWidth value={project} disabled
            InputProps={{ readOnly: true }} sx={{ '& .MuiOutlinedInput-root': { height: '40px' } }} />
          <TextField label="Task Name" fullWidth value={taskName} disabled
            InputProps={{ readOnly: true }} sx={{ '& .MuiOutlinedInput-root': { height: '40px' } }} />
        </Box>

        <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
          <TextField label="Enter SubTask" value={subtask} onChange={(e) => setSubtask(e.target.value)}
            sx={{ '& .MuiOutlinedInput-root': { height: '40px', width: '260px' } }} />
          <TextField label="Status" variant="outlined" value={status ? 'Open' : 'Closed'}
            InputProps={{
              readOnly: true,
              endAdornment: (
                <InputAdornment position="end">
                  <Box onClick={() => setStatus(!status)} sx={{
                    width: '40px', height: '20px', backgroundColor: status ? '#3DC1F2' : '#ccc',
                    borderRadius: '20px', position: 'relative', cursor: 'pointer', transition: 'background-color 0.3s',
                    mr: 4
                  }}>
                    <Box sx={{
                      height: '16px', width: '16px', backgroundColor: '#fff', borderRadius: '50%',
                      position: 'absolute', top: '2px', left: status ? '20px' : '3px', transition: 'left 0.3s',
                    }} />
                  </Box>
                </InputAdornment>
              ),
            }}
            sx={{ width: { xs: '100%', sm: '247px' }, '& .MuiOutlinedInput-root': { height: '40px', width: '260px' } }} />
        </Box>
        ...
        <Box display="flex" justifyContent="center" mt="auto">
          <Button variant="contained" onClick={handleSubmit} sx={{
            backgroundColor: "#3758f9", paddingX: 5, paddingY: 1, textTransform: 'none',
            width: { xs: '100%', sm: 'auto' }, '&:hover': { backgroundColor: "#2c47c5" },
          }}>
            Submit
          </Button>
        </Box>
